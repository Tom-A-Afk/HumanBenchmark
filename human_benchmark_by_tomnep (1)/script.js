...
function initBests(){
  ...
  qs('#sequence-best') && (qs('#sequence-best').textContent = getBest('sequence') ?? '—');
}
initBests();
...
function renderDashboard(){
  ...
  qs('#dash-sequence') && (qs('#dash-sequence').textContent = getBest('sequence') ?? '—');
}
...
(function ChimpTest(){
  ...
  function revealCell(pos){
    ...
    // reveal the original number for this position if part of the current sequence
    const seqIndex = sequence.indexOf(pos);
    el.textContent = seqIndex !== -1 ? (seqIndex + 1) : '';
  }
  ...
  board.addEventListener('click', (e) => {
    ...
    if (idx === expected){
      // correct
      // reveal the clicked number (its order in the sequence) and keep it visible
...
(function TypingTest(){
  ...
  const samples = [
    ...
    "Consistency and small improvements compound into impressive progress over time.",
    "A fast typist still needs to prioritize accuracy — mistakes slow you down overall.",
    "Short exercises done frequently are better than one long cram session.",
    "Focus on posture and comfortable hand placement to sustain speed for longer sessions."
  ];
  ...
  function concludeTest(){
    ...
    setBest('typing', wpm);
    qs('#typing-best').textContent = getBest('typing') ?? '—';
    // automatically advance to next sample after a short delay
    const nextIndex = (Number(sampleSelect.value) + 1) % samples.length;
    setTimeout(() => {
      sampleSelect.value = nextIndex;
  ...
  typingInput.addEventListener('keydown', (e) => {
    // Pressing Enter alone or Ctrl+Enter will finish the test
    if (e.key === 'Enter') {
...
(function SequenceTest(){
  const board = qs('#sequence-board');
  const startBtn = qs('#sequence-start');
  const message = qs('#sequence-message');
  const levelEl = qs('#sequence-level');

  let level = 1;
  let sequence = [];
  let playing = false; // when sequence is being shown
  let accepting = false; // when user can click
  let nextIndex = 0;
  let flashTimeouts = [];
  ...
  function resetBoardState(){
    qsa('.chimp-cell', board).forEach(c => { c.classList.remove('hidden','active'); c.textContent = ''; c.style.transform = ''; });
  }
  ...
  function newRound(){
    ...
    accepting = false;
    playing = true;
    // clear any previously lit cells from last round
    resetBoardState();
    sequence = generateSequence(level);
    levelEl.textContent = level;
    playSequence(sequence).then(() => {
      // after showing sequence, hide numbers and accept input
      hideAll();
      playing = false;
      accepting = true;
      nextIndex = 0;
  ...
  function playSequence(seq){
    return new Promise(resolve => {
      ...
      seq.forEach((pos, i) => {
        ...
        flashTimeouts.push(tOn, tOff);
      });
      const total = delay * (seq.length*2);
      const tEnd = setTimeout(() => resolve(), total + 80);
      flashTimeouts.push(tEnd);
    });
  }
  ...
  board.addEventListener('click', (e) => {
    ...
    if (!cell) return;
    const idx = Number(cell.dataset.index);
    // reveal clicked cell visually (light up briefly then hide)
    cell.classList.remove('hidden');
    cell.classList.add('active');
    cell.style.transform = '';
    // auto-hide the clicked cell after a short flash
    const t = setTimeout(() => {
    ...
    if (idx === expected){
      ...
      if (nextIndex === sequence.length){
        ...
        accepting = false;
        level++;
        setBest('sequence', level-1);
        qs('#sequence-best').textContent = getBest('sequence') ?? '—';
  ...
  // reset when leaving view
  const seqView = qs('#view-sequence');
  const observer = new MutationObserver(() => {
    if (!seqView.classList.contains('active')) {
      accepting = false;
      playing = false;
      message.textContent = '';
      clearFlashes();
  ...
  observer.observe(seqView, { attributes:true, attributeFilter:['class'] });
